...
router.post('/close', auth, authorize('director', 'cashier'), idempotency(), auditLog('CASH_CLOSE', 'cash_sessions'), ctrl.closeSession);
router.post('/paid-in', auth, authorize('director', 'cashier'), idempotency(), auditLog('CASH_PAID_IN', 'cash_movements'), ctrl.paidIn);
router.post('/paid-out', auth, authorize('director', 'cashier'), idempotency(), auditLog('CASH_PAID_OUT', 'cash_movements'), ctrl.paidOut);
