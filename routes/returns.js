...
router.post('/', auth, authorize('director', 'cashier'), idempotency(), auditLog('CREATE', 'returns'), ctrl.createReturn);
router.patch('/:id/process', auth, authorize('director'), idempotency(), auditLog('PROCESS_RETURN', 'returns'), ctrl.processReturn);
